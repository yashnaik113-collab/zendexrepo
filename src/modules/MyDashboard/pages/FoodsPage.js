...
const FoodsPage = () => {
  ...
  const handleToggle = async (dish, isAvailable) => {
    try {
      ...
      showToast(
        `${dish.foodName} ${isAvailable ? "is back in stock" : "is now out of stock"}`,
        "success",
      );
    } catch (error) {
      showToast(error.message, "error");
      ...
      >
        <Box>
          ...
          <Typography sx={{ color: "rgba(226,232,240,0.72)" }}>
            Add, edit, delete, and toggle dishes here. Customer menu pages pick
            up backend changes on the next sync.
          </Typography>
        </Box>
        <Button
